...
- Clean data point that are meaningful
In the raw data, we observe pics before 0.5 (blue line) and after 14 (red line). Those pics will be excluded to eliminate false detection during set up, washing and equilibrating the HPLC.
```{r look at retention time}
omics_data %>% 
...
omics_data %>% 
  ...
  #   row_retention_time <= 0.5    ~ "≤ 0.5",
  #   row_retention_time >= 14     ~ "≥ 14",
  #   TRUE                         ~ "included"
  # )) %>% 
  ggplot(aes(x= row_retention_time, y=row_m_z#, color= non_heavy_identified_flag
             ))+
  geom_point()+
  theme_classic()
```
...
```
We have `r nrow(clean_metabolites)` unique metabolites in the data. `r nrow(clean_metabolites %>% filter(charge == "neg"))` are negatively charged. and `r nrow(clean_metabolites %>% filter(charge == "pos"))` are positively charged.
...
mldata <- mldata %>% 
  ...
  dplyr::select(hmdb, row_m_z, row_retention_time, 
                is_lipids,
                starts_with("t"), -taxonomy_super_class)
```

```{r}
unknown_metabolites <- unknown_metabolites %>% 
  mutate(across((starts_with("t")), .fns = ~ replace_na(., min(., na.rm = TRUE)))) %>% 
  mutate_if(is.character, factor)
```
...
<!-- map_df1 <- map_df %>% -->
<!--   tidyr::unite(metabolite, c(hmdb, is_lipids), sep = "_", remove = TRUE) %>% -->
