...
```{r data prep}
# full_data <- full_data %>% 
#   mutate(is_lipids = relevel(is_lipids, "Yes"))
...
The best models:  
- Decision Tree with the highest accuracy. Area under the curve for the precision-recall and metrics sensitivity are not very high, but still good good. Moreover, it displays one of the highest specificity.  
- Random forest with the second highest accuracy. Random Forest metrics follow the values for the decision tree model but with a better area under the curve for the precision-recall metrics.   
- Trimmed Decision Tree with the seventh highest accuracy. **Trimmed decision tree show an area under the curve for the precision-recall metric a little higher for No lipid prediction but a lower value for Yes predictions. The sensitivity is within the highest, and the specificity metric is within lowest.**  
...
## 2.Evaluate the best models (best accuracy) in depth on full training set
```{r fit_resamples}
set.seed(1234)
...
n2_tree_results %>% # Compare both models
  ...
  scale_fill_manual(values = c("#8DA0CB", "#FC8D62", "#B2DF8A", "#66C2A5"), 
                    name= NULL)
  # scale_fill_viridis_d(option = "A", begin= 0.2, end = 0.8)
```
<span style="color:green">The 4 models show a really good performance for all metrics but specificity. The sensitivity (ability to detect a true positive) is really good but the models suffer from small number of lipid observation in the data which leads to a lower specificity (No lipids are classified as lipids). This is especially observed for the trimmed decision tree. Of note, ROC_AUC is also lower for the trimmed decision tree but it is not the best metric to make a decision in the case of unbalanced data.</span>
...
<span style="color:green">While the Trimmed decision tree doesn't exhibit the best PR_AUC for No lipid prediction, it has the best PR_AUC for Yes lipid prediction.</span> 
...
collect_metrics(final_fit) %>% 
  ...
  summarize(performance = mean(.estimate),
            std_err = sd(.estimate)) %>% 
  mutate(metric = "pr_auc_No")) %>% 
    bind_rows(., n2_tree_results %>% # Compare both models
  collect_predictions(summarize = FALSE) %>%
  group_by(id) %>% 
  pr_auc(is_lipids, .pred_Yes) %>% 
  ungroup() %>% 
...
# VI. Prediction on validation data 3990 with Trimmed Decision Tree
## 1. Cleaning
...
<!-- ### Lipids -->
<!-- What is the proportion of lipids in the validation data -->
<!-- ```{r} -->
<!-- validation3990_data %>% -->
<!--   ggplot(aes(x=is_lipids, fill= is_lipids))+ -->
<!--   geom_bar() -->
...
## 3. Apply our model to the validation data
```{r predict new data}
...
collect_metrics(all_workflows) %>%
  ...
  geom_errorbar(aes(ymin = mean-std_err, ymax = mean+std_err), color= "#66C2A5") +
