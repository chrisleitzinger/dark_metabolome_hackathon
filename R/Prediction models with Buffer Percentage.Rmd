...
In the raw data, we observe metabolites before 0.5 and after 14 for row_retention_time (red rectangles). These metabolites are excluded to eliminate false detection during set up, washing and equilibrating the HPLC. As you can see in the histogram, the pics before and after the blue lines were removed.
<div class = "row">
<div class = "col-md-6">
```{r}
omics1799_data %>% 
...
- Decision_Tree with an accuracy of `r as.character(round(performance_workflows %>% distinct(Model_Type, .keep_all = TRUE) %>% filter(Model_Type == "Decision_Tree") %>% dplyr::select(mean),2))`.  
...
n2_tree_results <- final_n2_tree %>%
  fit_resamples(
    ...
    metrics = metric_set(roc_auc, accuracy, sensitivity, specificity, precision),
...
<span style="color:green">The decision tree exhibit a good PR.</span>
<br>
<br>
...
```
The model is not overfitting (performance with the testing set is not lower than with the training set).
```{r}
collect_metrics(final_fit) %>% 
...
As a reminder, the same model trained on the dataset containing m/z and retention time only had an average accuracy of `r as.character(round(two_predictor_perfmean),2)`+/- `r as.character(round(two_predictor_perfsd),2)` showed in the grey dotted line and rectangle.  
